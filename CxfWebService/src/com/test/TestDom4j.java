...
public class TestDom4j {
	...
	public static void testParseXMLData(String xmlFileName) {
		...
		for (Iterator i_pe = root.elementIterator(); i_pe.hasNext();) {
			...
			for (Iterator i_adds = e_adds.elementIterator(); i_adds.hasNext();) {
				...
				sb.append("\t\t" + e_add.getName() + ":" + " code=" + code
						+ " value=\"" + add + "\"\n");
			}
			sb.append("\n");
		}
	...
	public static void testParseXMLSoapData(String xmlFileName){
		// 将xml文档转换为Document的对象
		Document document = parse2Document(xmlFileName);
		// 获取文档的根元素
		Element root = document.getRootElement();
		...
		for (Iterator i_pe = root.elementIterator(); i_pe.hasNext();) {
			Element e_pe = (Element) i_pe.next();
			// 获取当前元素的名字
			String name = e_pe.getName();
			System.out.println(name);
			
			// 获取当前元素e_pe(在此是person元素)下的子元素adds
			Element e_adds = e_pe.element("ns2");
...
//		testParseXMLSoapData("/soap.xml");
		
		
		SAXReader reader = new SAXReader();
		String url = "http://localhost:9999/hello/sayHello?name=liuxiaoling";
		Document doc;
		try {
			doc = reader.read("c:/soap.xml");//"c:/soap.xml"
